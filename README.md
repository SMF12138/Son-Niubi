...
**按信号强度分档（校准表，与 `calibration.json` 一致）：**

| | z>1.5（极强）| z>1.0 | z>0.5 | z≤0.5（弱）|
|---|---|---|---|---|
| N=7 | **79.2%** | 69.7% | 69.7% | 61.8% |
| N=30 | **74.8%** | 64.9% | 64.9% | 56.2% |
| N=60 | **76.0%** | 74.1% | 64.7% | 53.5% |
| N=90 | **76.3%** | 73.6% | 58.4% | 49.5% |

> **z>1.0 与 z>0.5 同值不是笔误**：N=7 实测 z>1.0 档 64.5%(n=124) 反而低于 z>0.5 档 72.1%(n=269)，两比例检验 |z|=1.49 → **统计上不可区分**。此时按**样本量加权池化**（合并 n=393 → 69.7%），双向收敛且方差更小（N=30 同理 → 64.9%）。
