...
if not exist "%SHORTCUT%" (
  powershell -NoProfile -Command "$r=$env:APPROOT.TrimEnd('\'); $w=New-Object -ComObject WScript.Shell; $s=$w.CreateShortcut($env:SHORTCUT); $s.TargetPath=Join-Path $r 'start.bat'; $s.WorkingDirectory=$r; $s.WindowStyle=7; $s.Save()" >nul 2>&1
)
